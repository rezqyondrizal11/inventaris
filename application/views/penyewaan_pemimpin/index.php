...
<div class="container-fluid">
    ...
    <a href="<?= site_url('penyewaan_pemimpin/print_pdf/' . $id . '?start_date=' . $this->input->get('start_date') . '&end_date=' . $this->input->get('end_date')) ?>" class="btn btn-danger mb-4" target="_blank">Print PDF</a>
    <a href="<?= site_url('penyewaan_pemimpin/export_excel/' . $id) ?>?start_date=<?= $this->input->get('start_date') ?>&end_date=<?= $this->input->get('end_date') ?>"
        class="btn btn-success mb-4">Export Excel</a>
    <!-- Filter Form -->
    <form method="GET" action="<?= site_url('penyewaan_pemimpin/index/' .  $id) ?>" class="mb-4">
        <div class="form-row">
            <div class="col-md-4">
